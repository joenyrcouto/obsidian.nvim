...
---@field id integer|? ID da marca no buffer
---@field row integer 0-indexed row
---@field col integer 0-indexed col
...
---Coleta marcas existentes em uma região
...
---Limpa marcas em um intervalo de linhas
...
---Limpa marcas em uma linha específica
...
local function get_line_ref_extmarks(marks, line, lnum, ui_opts, client)
  ...
  for match in iter(matches) do
    ...
    -- Lógica de Link Quebrado vs Válido
